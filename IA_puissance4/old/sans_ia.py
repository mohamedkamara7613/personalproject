...
def draw_board(game):
    """Dessine la grille et les points sur la console(termninal)"""
    
    symbols = {VIDE: '.', JOUEUR_1: "\033[31mX\033[0m", JOUEUR_2: "\033[33mO\033[0m"}
    ...
    print("-" * (4 * COLONNES - 1))  # Ajuster la taille du séparateur horizontal
...
def main():
    ...
    while True:
        print(f"\n\033[32mJoueur {game.current_player} ({'X' if game.current_player == JOUEUR_1 else 'O'}), à vous ! ٩(˘◡˘)۶\033[0m\n")
        try:
            column = int(input("Choisissez une colonne (0-6) : "))

            if game.drop_piece(column):
                draw_board(game) # dessiner la grille
                ...
                print("\n \033[31m Cette colonne est pleine ou invalide. Essayez encore. ¯\_( ͡❛ ͜ʖ ͡❛)_/¯\033[0m\n")
        except ValueError:
